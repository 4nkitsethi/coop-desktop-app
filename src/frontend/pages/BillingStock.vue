...
<div class="SplitPnl SplitPnl-Middle Text--Center"> 
  ...
  <div class="TitelHr pt--2 pb--2 pl--2 pr--2">
    ...
    <a href="javascript:void(0);" class="Btn-Normal DropShadow--Normal Text--Dark-Green pl--2 pr--2 ml--18" id="NewRequest" @click.prevent="currentStockTab=false;newStockTab=true;updateProgress()">{{ $t('NewStockRequest') }}</a> 
  ...
  <div class="MiddleGridPnl" id="RequestProductPnl" v-show="currentStockTab">
    ...
    <div class="Bottom-ProductPnl">
      <div class="MainProductPnl">                
        <!--Product Item-->
        <div class="Product-Item DropShadow--Normal Overflow--hidden ma--15" v-for="product in products" :key="product.id">
            ...
            <div class="Flip-Box-shadow Main-Flip-Box" :ref="'product-panel-'+product.id" @click="this.$refs['product-panel-'+product.id][0].classList.toggle('Active')">
              <div class="Main-chart" >
                <span :class="'current-stock-chart-'+product.id" :data-percent="(product.association.stock/product.association.totalQtyPerDay) * 100"  style="display: flex;justify-content: center;align-items: center;" > 
                  <span class="Info-Text">{{$t('Stock')}}</span> <span class="percent" >{{ scale(product.association.stock,product.digits) }} <small>{{ product.weight_unit  }}</small></span> 
                </span>
              </div>
  ...
  <div class="MiddleGridPnl" id="newStockPnl" v-show="newStockTab">
    ...
    <div class="Bottom-ProductPnl">
      <div class="MainProductPnl"  >                 
        <!--Product Item-->
        <div class="Product-Item DropShadow--Normal Overflow--hidden ma--15"  v-for="product in newStockRequest" :key="product.id">
          <div class="Product-Type Product-Type-Top pa--1" >
            <div class="Type-Left" id="MainProduct"> <img :src="product.raw_image" class="logo mb--1">
              <h4>{{ product.product_name }}</h4>
            </div>
            <div class="Flip-Box-shadow Main-Flip-Box" :ref="'new-product-panel-'+product.id" @click="this.$refs['new-product-panel-'+product.id][0].classList.toggle('Active')">
              <div class="Main-chart">
                <span :class="'new-current-stock-chart-'+product.id" :data-percent="(product.association.stock/product.association.totalQtyPerDay) * 100" style="display: flex;justify-content: center;align-items: center;"> 
              ...
              <div class="Flip-chart">
                <span :class="'new-current-sale-chart-'+product.id" :data-percent="(product.association.totalQtySaleToday/product.association.totalQtyPerDay) * 100" style="display: flex;justify-content: center;align-items: center;"> 
                  <span class="Info-Text">{{ $t('Sale') }}</span> 
                  <span class="percent" >{{ currency(product.association.todaySales) }}</span> 
                </span>
              </div>
          ...
          <div class="Product-Type Product-Dtls Box--inline-shadow pa--1" >
            <div class="Inputflex">
              <input name="amount" type="text" class="inputStyle Stock-Req-Field" v-model="product.requested_stock" v-maska  :data-maska="product.mask" data-maska-tokens="*:[0-9]:multiple"  placeholder="0" />
              ...
              <div :id="'collapseOne'+index" class="accordion-collapse collapse" :aria-labelledby="'headingOne'+index" data-bs-parent="#accordionExample">
                <div class="accordion-body content pb--1">
                        ...
                        <td style="padding:0" v-if="['Requested','Approved'].includes(sr.status)">
                          <div class="Inputflex">
                            <template v-if="sr.editMode && rp.status != 'Cancelled'">
                              <input name="amount" type="text" class="inputStyle Edit-Qty Act" v-model="rp.stock_request" @input="rp.total = parseFloat(rp.supply_rate) * parseFloat(rp.stock_request); rp.$save()">
                  ...
                  <div class="tableBtnGrp" v-if="sr.status == 'Requested'"> 
                    <a href="javascript:void(0);" class="Btn-Normal DropShadow--Normal pl--24 pr--24 Text--Uppercase mr--1 Edit-Stock-Qty-Btn" v-if="!sr.editMode" @click="sr.editMode = true;sr.$save();"> {{ $t('Edit') }}</a> 
            ...
            <tbody class="d--block">
              <tr class="d--block" v-for="req in receiveStockRequest.requested_products" :key="req.id">
                ...
                <td class="pt--1 pr--24 pb--1 pl--24 Width--15">
                  <div class="Inputflex">
                    <input name="amount" type="text" class="inputStyle number-input-text" id="number-input-text" placeholder="0" v-model="req.stock_received">
                    <!--<input id="number-input-text" class="inputStyle number-input-text" name="" type="tel" placeholder="Enter your phone no.">--> 
...
export default {
  ...
  methods:{   
          ...
          updateProgress(){
              this.products.filter( item => {
                ...
                if(elementStock){
                  if(!elementStock.querySelector("canvas")){
                    new EasyPieChart(elementStock, {
                ...
                if(elementSale){
                  if(!elementSale.querySelector("canvas")){
                    new EasyPieChart(elementSale, {
                        // your options goes here
                        barColor:'#bf8a00',
                        scaleColor:false,
                    })
                ...
                let elementNewStock = document.querySelector(".new-current-stock-chart-"+item.id);
                if(elementNewStock){
                  if(!elementNewStock.querySelector("canvas")){
                    new EasyPieChart(elementNewStock, {
                        // your options goes here
                        barColor:'#bf8a00',
                        scaleColor:false,
                    })
                ...
                let elementNewSale = document.querySelector(".new-current-sale-chart-"+item.id);
                if(elementNewSale){
                  if(!elementNewSale.querySelector("canvas")){
                    new EasyPieChart(elementNewSale, {
                        // your options goes here
                        barColor:'#bf8a00',
                        scaleColor:false,
                    })
  ...
  mounted(){            
            this.updateProgress();         
            this.products.filter( item => {
                                              item.requested_stock = parseFloat(0);
                                              this.newStockRequest.push(pick(item,['id','weight_unit','wholesale_weight_range','stock','status','supplier_id','requested_stock','product_name','image','mask','association']))
            })
